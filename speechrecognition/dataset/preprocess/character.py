...
def generate_character_labels(dataset_path, labels_dest):
    ...
    label_df.to_csv(os.path.join(labels_dest, "char_labels.csv"), encoding="utf-8", index=False)


def generate_character_script(dataset_path, new_path, script_prefix, labels_dest):
    print('create_script started..')
    char2id, id2char = load_label(os.path.join(labels_dest, "char_labels.csv"))
...
if __name__ == '__main__' :
    import os
    os.chdir('../../')
    generate_character_labels('../data/KsponSpeech/', '../data')